...
from objects.utils import (
    rebuildSession,
    flashrect,
    render_template_flashrect as render_template,
    flash,
...
async def forgot_reset_password():
    ...
    if error := validate_password(new_password):
        return await flash('error', f'Your new password {error[0].lower()}{error[1:]}', 'forgot')
...
async def settings_profile_post():
    ...
    log2.info(f"Profile update attempt: {old_name} -> {new_name} | Email change: {old_email != new_email}")
    ...
    if not success:
        return await flash('error', f'old Email verification code {error_msg[0].lower()}{error_msg[1:]}', 'settings/profile')    
    # Update username if changed
    if new_name != old_name:
        # Validate new username
        if error := validate_username(new_name):
            return await flash('error', f'Your new username {error[0].lower()}{error[1:]}', 'settings/profile')
    ...
    if new_email != old_email:
        ...
        if not success:
            return await flash('error', f'new Email verification code {error_msg[0].lower()}{error_msg[1:]}', 'settings/profile')
        
        # Validate new email
        if error := validate_email(new_email):
            return await flash('error', f'Your new email {error[0].lower()}{error[1:]}', 'settings/profile')
...
async def clanPage(id):
    ...
    return await render_template('clans/profile.html', clanInfo=clanInfo, clanMembers=clanMembers, mode=mode, mods=mods, datetime=datetime, timeago=timeago)
